...
int main() {
  ...
  while(testcases--) {
    ...
    for(auto ele:inp) {
      ...
      if(number==ele.first)
        cc = ele.second, count += ele.second;
      else
        cc = 0;

      //lli number = ele.first;
      lli temp = power(2LL, n-count)%MOD;
      temp = (mul%MOD * temp%MOD) %MOD;
      temp = (number%MOD * temp%MOD)%MOD;
      //ans += i*(mul*(power(2,n-count)) %MOD)%MOD;
      ans = (ans%MOD + temp%MOD)%MOD;
      ans = ans % MOD;
      temp = (power(2LL, cc)%MOD - 1LL%MOD)%MOD;
      mul = (mul%MOD * temp%MOD)%MOD;
      //mul *= (power(2, arr[i]) - 1 )%MOD;
      if(number!=ele.first) {
        //cout << "BREAKING at " << number <<" "<<ele.first<< endl;
        break;
      }
      ++number;
    }
